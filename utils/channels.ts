...
class Channels {
  ...
  static async add(channel: string) {
    const channels = await Channels.getAll();

    const channelExists = channels.some((prevChannel) => prevChannel === channel);
    if (channelExists) {
      return;
    }
  ...
  static async delete(channel: string) {
    let channels = await Channels.getAll();
    channels = channels.filter((prevChannel) => prevChannel !== channel);
